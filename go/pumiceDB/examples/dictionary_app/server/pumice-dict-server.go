...
func (dso *DictionaryServer) Apply(app_id unsafe.Pointer,
	...
	/* Iterate over word_map and write work as key and frequency as value
	 * to pmdb.
	 */
	for word, count := range word_map {
		...
		prev_result := dso.pso.LookupKey(word, int64(go_key_len),
			prev_value, colmfamily)
		...
		dso.pso.WriteKV(app_id, pmdb_handle, word, int64(go_key_len),
			value, int64(value_len), colmfamily)
...
func (dso *DictionaryServer) Read(app_id unsafe.Pointer,
	...
	/* Pass the word as key to ReadKV and get the value from pumicedb */
	result := dso.pso.ReadKV(app_id, req_dict.Dict_text, int64(key_len),
		colmfamily)
...
func dictionaryServerNew() *DictionaryServer {
	...
	flag.StringVar(&dso.raftUuid, "raft", "NULL", "raft uuid")
	flag.StringVar(&dso.peerUuid, "peer", "NULL", "peer uuid")

	flag.Parse()
	fmt.Println("Raft UUID: ", &dso.raftUuid)
	fmt.Println("Peer UUID: ", &dso.peerUuid)

	dso.columnFamilies = colmfamily
