...
type SerfAgentHandler struct {
	...
	RpcAddr  string //Addr[addr:port] for agent-client communication
...
func (Handler *SerfAgentHandler) start() error {
	...
	FuncHandler := &agent.ScriptEventHandler{
		...
		Logger:   Handler.AgentLogger, // Change the log here too
	}
	...
	rpcListener, err := net.Listen("tcp", Handler.RpcAddr)
	if err != nil {
		return err
	}
