...
func (handler *clientHandler) getCmdParams() {
	...
	flag.IntVar(&handler.serviceRetry, "sr", 1, "send how many times you want to retry the if proxy is not available")
	flag.Parse()
}
...
func main() {
	...
	fmt.Println("clientObj.serviceRetry", clientObj.serviceRetry)
	err = clientObj.clientAPIObj.TillReady("", clientObj.serviceRetry)
	if err != nil {
		log.Error(err)
		os.Exit(1)
	}
	...
	case "write":
		err := clientObj.clientAPIObj.TillReady("PROXY", clientObj.serviceRetry)
		if err != nil {
			log.Error(err)
			os.Exit(1)
		}
