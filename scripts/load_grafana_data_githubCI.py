...
def write_worflow_timings_influxdb(data, workflow_name):
    ...
    for workflow_run in data["workflow_runs"]:
        run_date = datetime.strptime(workflow_run["run_started_at"],"%Y-%m-%dT%H:%M:%S%z")
        # datetime.datetime(2020, 5, 17)
        # print(run_date, datetime.today().date())
        if run_date.date() >= datetime(2022, 7, 20).date():
            if workflow_run["name"] == workflow_name:   
                actions_url = "https://api.github.com/repos/00pauln00/niova-core/actions/runs/{}/jobs".format(str(workflow_run["id"]))
            ...
            if summary["failed"] != 0:
                ...
                recipe_body.append({
                    ...
                    "fields":{
                        ...
                        "job_id": str(influx_data["id"]),
                        "Task_1": top3_tasks[0] + " - " + str(tasks.get(top3_tasks[0])),
                        "Task_2": top3_tasks[1] + " - " + str(tasks.get(top3_tasks[1])),
                        "Task_3": top3_tasks[2] + " - " + str(tasks.get(top3_tasks[2]))
                    } 
                })
            # pprint(recipe_body)
        pprint(recipe_list)
...
def write_workflow_timings_local(data):
    ...
    for key in dict_values:
        ...
        myfile_path = date_time_path + "/" + dict_values[key][1] +"/" + dict_values[key][1] +"parsed.json"
        ...
        with open(myfile_path, 'a') as file:
            for job in work["jobs"]:
                ...
                workflow_entry = [WorkFlowStartTime, WorkFlowCompletedTime, WorkFlowConclusionTime]
...
workflow_names = ['holon_basic_recipes_workflow',
