...
#define LREG_NODE_INSTALL_LOCK   pthread_mutex_lock(&thr_lreg_handle.lrh_mutex)
#define LREG_NODE_INSTALL_UNLOCK pthread_mutex_unlock(&thr_lreg_handle.lrh_mutex)
...
{
    NIOVA_ASSERT(thr_lreg_handle.lrh_init);

    return &thr_lreg_handle.lrh_root;
}
...
lreg_install_get_queued_node(void)
{
    ...
    if (!CIRCLEQ_EMPTY(&thr_lreg_handle.lrh_installq))
    {
        install = CIRCLEQ_FIRST(&thr_lreg_handle.lrh_installq);
        CIRCLEQ_REMOVE(&thr_lreg_handle.lrh_installq, install, lrn_lentry);
    }
...
lreg_remove_get_queued_node(void)
{
    ...
    if ((remove = STAILQ_FIRST(&thr_lreg_handle.lrh_destroyq)))
        STAILQ_REMOVE_HEAD(&thr_lreg_handle.lrh_destroyq, lrn_removal_lentry);
...
lreg_notify(void)
{
    ...
    return write(thr_lreg_handle.lrh_eventfd, &val, sizeof(val));
}
...
lreg_install_has_queued_nodes(void)
{
    LREG_NODE_INSTALL_LOCK;
    bool empty =
        (CIRCLEQ_EMPTY(&thr_lreg_handle.lrh_installq) && STAILQ_EMPTY(&thr_lreg_handle.lrh_destroyq));
    LREG_NODE_INSTALL_UNLOCK;
...
lreg_node_remove_queue(struct lreg_node *child)
{
    ...
    STAILQ_INSERT_TAIL(&thr_lreg_handle.lrh_destroyq, child, lrn_removal_lentry);
...
{
    return thr_lreg_handle.lrh_eventfd;
}
...
{
    return util_thread_remove_event_src(thr_lreg_handle.lrh_eph);
}
void
...
lreg_subsystem_init(void)
{
    ...
    int rc = util_thread_install_event_src(thr_lreg_handle.lrh_eventfd, EPOLLIN,
                                           lreg_util_thread_cb, NULL, &thr_lreg_handle.lrh_eph);
    ...
    SIMPLE_LOG_MSG(LL_DEBUG, "hello lRegEventFD=%d", thr_lreg_handle.lrh_eventfd);
}
...
lreg_subsystem_destroy(void)
{
    ...
    SIMPLE_LOG_MSG(LL_DEBUG, "goodbye, svc thread lRegEventFD=%d", thr_lreg_handle.lrh_eventfd);

    if (thr_lreg_handle.lrh_eventfd >= 0)
    {
        int rc = close(thr_lreg_handle.lrh_eventfd);
        if (rc)
            SIMPLE_LOG_MSG(LL_ERROR, "close(lRegEventFD=%d): %s",
                           thr_lreg_handle.lrh_eventfd, strerror(-rc));
    }
}
