    ...
    enum raft_instance_newest_entry_hdr_types type)
{
         ...
         i < RI_NEHDR_ALL; i++)
    {
        ...
        if (reh->reh_index > tgt->reh_index)
        {
            updated = true;
            *tgt = *reh;
        }
                        ...
                        const struct raft_net_sm_write_supplements *ws)
{
    ...
    raft_instance_update_newest_entry_hdr(ri, &re->re_header, type);
 ...
 */
static int
raft_server_backend_sync(struct raft_instance *ri)
{
    if (!ri)
        return -EINVAL;
                             ...
                             const uuid_t candidate, int64_t candidate_term)
{
    ...
    return rc ? rc : raft_server_backend_sync(ri);
}
...
raft_server_entries_scan(struct raft_instance *ri)
{
    ...
    for (raft_entry_idx_t i = 0; i < num_entries; i++)
    {
         ...
         */
        raft_instance_update_newest_entry_hdr(ri, &reh, RI_NEHDR_ALL);
    }
    ...
    const struct raft_append_entries_request_msg *raerq)
{
    ...
    // Force reinit the header before applying an 'older' value
    raft_instance_initialize_newest_entry_hdr(ri);
...
raft_server_leader_calculate_committed_idx(struct raft_instance *ri)
{
    ...
    self->rfi_synced_idx = sync_hdr.reh_index + 1;
    ...
    for (raft_peer_t i = 0; i < num_raft_members; i++)
    {
        ...
        for (raft_peer_t j = 0; j < num_raft_members; j++)
        {
            ...
            {
                sorted_indexes[i] = rfi->rfi_synced_idx;
                tmp_peer = j;
            }
        }
    ...
    const struct raft_append_entries_reply_msg *raerp)
{
    ...
    DBG_RAFT_INSTANCE(
        ...
        "follower=%x next-idx=%ld si=%ld err=%hhx rp-pli=%ld rp-si=%ld",
...
//XXX what about synced_idx here?
    ...
    else
    {
        ...
        {
            DBG_RAFT_INSTANCE(LL_NOTIFY, ri, "follower=%x new-sync-idx=%ld",
                              follower_idx, rfi->rfi_synced_idx);

            rfi->rfi_synced_idx = raerp->raerpm_synced_log_index;

            // Only called if the follower's sync index has increased
